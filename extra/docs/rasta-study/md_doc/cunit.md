...
## Running the tests
In Order to run all registered CUnit tests you build the project using the legacy Gradle workflow (see [HowTo Gradle](gradle.md)).  
...
All unit test files are placed in `tests/rasta/`. To be more specific, the source files (.c) are in the subdirectory `c/` and the header files (.h) are in `headers/`. You have to create a header file although you might not need it for your unit tests. It is needed to register the tests later on.
Let's create a simple unit test. We start by creating a header file `mytest.h` in `tests/rasta/headers/` with the following content:
```
